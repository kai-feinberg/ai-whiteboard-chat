...
export const listCanvasesWithChats = query({
  ...
  handler: async (ctx) => {
    ...
    const canvases = await Promise.all(canvasIds.map((id) => ctx.db.get(id)))

    // Filter out nulls, verify org ownership, and sort by updatedAt descending
    return canvases
      .filter(
        (c): c is NonNullable<typeof c> =>
          c !== null && c.organizationId === organizationId,
      )
      ...
      nodes.map(async (node) => {
        if (node.nodeType === 'text') {
          const textNode = await ctx.db.get(
            node.data.nodeId as Id<'text_nodes'>,
          )
          return {
            ...node,
          ...
          const chatNode = await ctx.db.get(
            node.data.nodeId as Id<'chat_nodes'>,
          )
          if (!chatNode) {
            return {
              ...node,
          ...
          const youtubeNode = await ctx.db.get(
            node.data.nodeId as Id<'youtube_nodes'>,
          )
          return {
            ...node,
          ...
          const websiteNode = await ctx.db.get(
            node.data.nodeId as Id<'website_nodes'>,
          )
          return {
            ...node,
          ...
          const tiktokNode = await ctx.db.get(
            node.data.nodeId as Id<'tiktok_nodes'>,
          )
          return {
            ...node,
          ...
          const twitterNode = await ctx.db.get(
            node.data.nodeId as Id<'twitter_nodes'>,
          )
          return {
            ...node,
          ...
          const facebookAdNode = await ctx.db.get(
            node.data.nodeId as Id<'facebook_ads_nodes'>,
          )
          return {
            ...node,
          ...
          const groupNode = await ctx.db.get(
            node.data.nodeId as Id<'group_nodes'>,
          )
          return {
            ...node,
          ...
          const imageNode = await ctx.db.get(
            node.data.nodeId as Id<'image_nodes'>,
          )
          return {
            ...node,
...
export const getFacebookAdNode = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (
      facebookAdNode.imageStorageIds &&
      facebookAdNode.imageStorageIds.length > 0
    ) {
      imageUrls = await Promise.all(
        facebookAdNode.imageStorageIds.map((storageId) =>
    ...
    if (facebookAdNode.videoThumbnailStorageId) {
      videoThumbnailUrl = await ctx.storage.getUrl(
        facebookAdNode.videoThumbnailStorageId,
      )
    }
...
export const createCanvas = action({
  ...
  handler: async (
    ctx,
    args,
  ): Promise<{ canvasId: Id<'canvases'>; title: string }> => {
    const identity = await ctx.auth.getUserIdentity()
    if (!identity) {
      throw new Error('Not authenticated')
    }
    ...
    // ========== AUTO-CREATE CHAT NODE ==========
    // Every new canvas gets a chat node automatically
    try {
      ...
      await ctx.runMutation(internal.canvas.nodes.createChatNodeInternal, {
        canvasId,
        position: { x: 100, y: 100 }, // Default position at top-left
        agentThreadId,
        userId,
        organizationId,
      })
    } catch (error) {
      // Log error but don't fail canvas creation if chat node creation fails
      console.error('[createCanvas] Failed to auto-create chat node:', error)
    }
