...
import {
  Edge as CustomEdge,
  DeleteButtonEdge,
} from '@/components/ai-elements/canvas/edge'
...
function CanvasEditor() {
  ...
  const createFacebookAdNode = useAction(
    api.canvas.facebook.createFacebookAdNode,
  )
  ...
  const handleNodeUngrouped = useCallback(
    (nodeData: any) => {
      console.log(
        '[Canvas] Node ungrouped, adding back to React Flow:',
        nodeData,
      )
  ...
  const handleTitleSave = async () => {
    if (titleValue.trim() && titleValue !== canvasData?.canvas?.title) {
      try {
        ...
        console.error('[Canvas] Error updating title:', error)
  ...
  useEffect(() => {
    if (hasLoadedFromDB && canvasData?.nodes) {
      ...
      const dbNodeIds = new Set(
        canvasData.nodes.filter((n) => !n.parentGroupId).map((n) => n._id),
      )
      ...
      const deletedNodeIds = Array.from(currentNodeIds).filter(
        (id) => !dbNodeIds.has(id),
      )
      if (deletedNodeIds.length > 0) {
        console.log(
          '[Canvas] Removing deleted nodes from local state:',
          ...
          addEdge(
            {
              ...connection,
              type: 'animated',
              data: { deleteEdge, isHovered: false },
            },
            eds,
          ),
        )
  ...
  const handleTikTokUrlSubmit = async (url: string) => {
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : 'Failed to create TikTok node',
      )
    }
  }
  ...
  const handleFacebookAdInputSubmit = async (input: string) => {
    ...
    if (urlMatch) {
      ...
      if (!numericMatch) {
        toast.error(
          'Please enter a valid Facebook Ad Library URL or numeric Ad ID',
        )
        return
      }
    }
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : 'Failed to create Facebook Ad node',
      )
    }
  }
      ...
      <div className="h-full w-full overflow-hidden rounded-2xl">
        ...
        >
          <Panel position="top-left">
            <div className="bg-accent/30 backdrop-blur-sm rounded-xl p-3 border border-border/50">
              {isEditingTitle ? (
                <Input
                  ref={titleInputRef}
                  value={titleValue}
                  ...
                  className="h-8 text-lg font-semibold bg-transparent border-0 focus-visible:ring-0 focus-visible:ring-offset-0 px-0"
                />
              ) : (
                <h1
                  onClick={handleTitleClick}
                  className="text-lg font-semibold truncate max-w-[300px] cursor-pointer hover:text-primary transition-colors select-none"
                  title="Click to edit"
                >
                  {canvasData?.canvas?.title || 'Untitled Canvas'}
                </h1>
              )}
            </div>
          </Panel>
